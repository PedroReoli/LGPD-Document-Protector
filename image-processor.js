...
class ImageProcessor {
    ...
    return new Promise((resolve, reject) => {
      ...
      setTimeout(() => {
        try {
          ...
          this.loadPDFPage(1).then(resolve).catch(reject)
        } catch (error) {
          this.isProcessing = false
  ...
  drawToCanvas(ctx, intensity, iterations, scaleFactor, offsetX, offsetY) {
    ...
    if (this.maskCanvas && hasMask) {
      ...
      if (!this.blurCache || this.lastBlurIntensity !== intensity || this.lastBlurIterations !== iterations) {
  ...
  applyBlurToSensitiveRegions(regions) {
    ...
    regions.forEach((region) => {
      this.addRectangleToMask(region.x, region.y, region.x + region.width, region.y + region.height)
    })
